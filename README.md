...
- **横短横线时间轴**：只在该次用户输入的位置显示一条横向短横线（浅灰），无消息处无线；所有条固定间距（14px）、条数等于**整个会话**的用户消息数，放得下时聚拢居中，放不下时时间轴内部滚动（无滚动条）。
- **全量统计（投影机制）**：通过 DSH sessionProjections 增量 fold 整个会话的用户消息 + AI 回复预览，持久化缓存保证刷新秒出、新消息实时更新；消息文本按紧凑档截断（用户消息 60 字符 / 回复 160 字符），长会话压力可控。
- **当前消息定位（scroll-spy）**：左侧**激活条**始终对应右侧视口当前显示的那条用户消息；滚动右侧对话，左侧激活条随之移动（用户手动滚动时间轴后保持不动，点击某条跳转后恢复跟随）。
- **波浪聚焦**：鼠标在时间轴上移动时，鼠标接近的那条变为激活色并变长（12→30px），相邻条长度向上下递减，约 2.5 条之外恢复原长，形成波浪效果。
- **圆角预览 tooltip**：鼠标移到某条时立即显示圆角 tooltip——第一行用户消息（黑色加粗、单行省略），下方 AI 回复（灰色、最多 3 行），时间贴在最后一行右下角；字体与右侧用户消息一致（16px），宽度约 20 个中文字。窗口外的历史消息也能预览（来自投影缓存）。
- **点击跳转**：点击任意横线，对话立即滚动到该次用户输入所在位置；窗口外（未加载）的历史消息会自动加载更早历史后跳转。
- **收起 / 展开**：第一条消息上方 1.3 倍间距处有一个悬停淡入的**胶囊把手**（点击收起；悬停时第一、二条灰色递减，胶囊扮演激活条角色）。收起后变成一条**全高细竖条**，常态隐藏，鼠标接近识别区域时淡入，点击展开。任何滚动位置，胶囊与最上方条目之间保持 1.3 格空白。
...
```
dsh-session-timeline/
├── package.json        # dsh.bundle + dsh.client 声明（依赖 zod、sessionProjections）
├── cordis.patch.yml    # bundle patch 层：插入插件行（inject: sessionProjections）
├── index.js            # host 半部：注册 timelineUserMessages 投影（增量 fold + 截断）
├── client.js           # 浏览器半部（__ModuleLoader__ bundle：UI 全部实现）
├── docs/               # 文档与展示素材（screenshots/，内含 README 说明）
└── README.md
```
